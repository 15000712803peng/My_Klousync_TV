...
import com.kloudsync.techexcel2.adapter.ChangeCountryCodeAdapter;
import com.kloudsync.techexcel2.changecode.CountryCodeShow;
...
import com.kloudsync.techexcel2.tool.PinyinComparatorCC;
...
public class ChangeCountryCode extends Activity {
	...
	private void GetLvListen() {
		lv_group.setOnKeyListener(new View.OnKeyListener() {
			@Override
	...
	private void ShowSelectItem() {
		int position = lv_group.getSelectedItemPosition();
		cadapter.updateItem(position);
	}
				...
				for (int i = 0; i < mlist.size(); i++) {
					...
					if ((getName.contains(nameb.toString()) || getCode
						...
						String url = name1 + "<font color=\"#55CF6F\">" + name2
								+ "</font>" + name3;
						ci.setShowname(url);						
						ci.setNcshow(getName.contains(nameb.toString()) ? true
								: false);
						eList.add(ci);
					}
				}
