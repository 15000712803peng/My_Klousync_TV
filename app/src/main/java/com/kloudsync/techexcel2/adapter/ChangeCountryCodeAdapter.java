...
import com.kloudsync.techexcel2.changecode.CheckCountryHelp;
import com.kloudsync.techexcel2.changecode.CountryCodeList;
...
public class ChangeCountryCodeAdapter extends CommonAdapter<CountryCodeInfo> {
    ...
    public void updateItem(int select) {
        this.select = select;
        updateAdapter(list);
    }
...
//			holder.setImageResource(R.id.img_country, R.drawable.country86);
        ...
        holder.setLinearBackgroundColor(R.id.lin_item,
                mContext.getResources().getColor((position == select) ? R.color.green : R.color.white));
