...
public class MyKlassroomActivity extends Activity implements View.OnClickListener {
    ...
    private void getClassRoomLessonID(final String classRoomId) {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    JSONObject jsonObject = ConnectService.getIncidentbyHttpGet(AppConfig.URL_PUBLIC + "Service/GetClassRoomLessonID?classRoomID=" + classRoomId);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    JSONObject jsonObject = ConnectService.submitDataByJson(AppConfig.URL_PUBLIC + "Service/AddInstantLesson?classroomID=" + classRoomId, js);
    ...
    private void getUpcomingLessonList(final String teacherid) {
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    JSONObject jsonObject = ConnectService.getIncidentbyHttpGet(AppConfig.URL_PUBLIC + "Service/UpcomingLessonList?teacherID=" + teacherid);
        ...
        switch (view.getId()) {
            ...
            case R.id.joinmycourse2:
                if (TextUtils.isEmpty(AppConfig.ClassRoomID)) {
                    Toast.makeText(MyKlassroomActivity.this, "你加入的会议不存在!", Toast.LENGTH_LONG).show();
                } else {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    JSONObject returnjson = ConnectService.submitDataByJson(AppConfig.URL_PUBLIC + "Service/CreateLesson", submitjson);
