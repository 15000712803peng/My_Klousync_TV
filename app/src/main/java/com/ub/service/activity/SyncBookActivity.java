...
import com.kloudsync.techexcel2.help.ShareKloudSyncDialog;
...
import com.kloudsync.techexcel2.tool.SyncRoomOutlinePopup;
import com.lidroid.xutils.HttpUtils;
import com.lidroid.xutils.util.PreferencesCookieStore;
import com.ub.service.activity.SyncBookOutlineManager.OnChapterClickedListener;
...
public class SyncBookActivity extends BaseActivity implements View.OnClickListener, AGEventHandler, VideoGestureRelativeLayout.VideoGestureListener, AddFileFromFavoriteDialog.OnFavoriteDocSelectedListener, AddFileFromDocumentDialog.OnDocSelectedListener, SyncRoomOutlinePopup.OnChapterClickedListener, SyncRoomOutlinePopup.OutlinePopupEventListener {
    ...
    private LineItem currentShowPdf;
    ...
    private RelativeLayout displayAudience, displayOutline, displaychat, displaywebcam, displayVideo, setting, setting2, displayrecord;
    ...
    private RelativeLayout syncdisplayoutline, syncdisplaymembers, syncdisplaymeeting, syncdisplaychat, syncdisplayproperty, syncdisplayshare, syncyinxiang, syncdisplayquit;
    ...
    public static SyncBookActivity instance = null;
        ...
        public void handleMessage(Message msg) {
            ...
            if (activity3 != null) {
                ...
                switch (msg.what) {
                    ...
                    case 0x1120:
                        final String m = (String) msg.obj;
                        Log.e("--------9", activity3.currentAttachmentId + "   " + activity3.currentAttachmentPage + "   " + activity3.currentMode + "  " + m);
...
//                        activity3.promoteAuditor((String) (msg.obj));
    ...
    private void changedocumentlabel(LineItem lineItem) {
        Log.e("dddddd大小", documentList.size() + " ");
        if (documentList.size() > 0) {
            if (lineItem == null) {
                Log.e("dddddd大小1", documentList.size() + " ");
                lineItem = documentList.get(0);
                lineItem.setSelect(true);
            } else {
                Log.e("dddddd大小2", documentList.size() + " ");
            ...
            Log.e("dddddd", currentAttachmentId + " " + newPath);
            Log.e("check_url", "one,targetUrl:" + targetUrl + ", new path:" + newPath + ",currentAttachmentId:" + currentAttachmentId);
            if (wv_show == null) {
                Log.e("check_null", "wx_show:" + (wv_show == null));
                return;
            }
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Log.e("check_null", "load file:///android_asset/index.html" + (wv_show == null));
...
//                notifySwitchDocumentSocket(lineItem, "1");
    ...
    private void changedocumentlabel(LineItem lineItem, List<LineItem> documentList) {
        Log.e("dddddd大小", documentList.size() + " ");
...
//        NotifyActivity.instance.ShowWhite();
        ...
        Log.e("SyncRoomActivity", "on create:" + targetUrl + "  " + "" + "  meetingId: " + meetingId + "  identity : " + identity + "    isInstantMeeting :" + isInstantMeeting + "   lession: " + lessonId);
...
//-------------------
    ...
    private void doJOIN_MEETING(String msg) {
        try {
            ...
            Log.e("SyncRoomActivity", "currentItemId:" + currentItemId);
...
//
//                            }else {
//                                if(!syncRoomOutlinePopup.isShowing()){
//                                    syncRoomOutlinePopup.
//                                }
//                            }
...
//                currentAttachmentId=getRetCodeByReturnData2("documentId", msg);
            ...
            if (msg_action.equals("BROADCAST_FRAME")) {
                ...
                Log.e("BROADCAST_FRAME", "msg:" + msg2);
            ...
            if (msg_action.equals("MAKE_PRESENTER")) {
                if (getRetCodeByReturnData2("retCode", msg).equals("1")) {  // 此时设置student or teacher presenter success
                    ...
                    Log.e("currentPresenterId ", currentPresenterId + "  " + AppConfig.UserID);
...
//            if (msg_action.equals("BIND_TV_JOIN_MEETING") && isTeamspace) {
//                isTeamspace = false;
//                lessonId = getRetCodeByReturnData2("retData", msg);
//                openshengwang(0);
...
//            }
    ...
    private void initView() {
        ...
        displayOutline = (RelativeLayout) findViewById(R.id.displayOutline);
        displayOutline.setOnClickListener(this);
...
//        getLineAction(currentAttachmentPage, true);
    ...
    public void afterLoadPageFunction() {
        ...
        Log.e("afterLoadPageFunction", "当前文档信息：url  " + targetUrl + "        " + crpage + "      newpath  " + newPath);
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                JsonDown();
    ...
    private boolean isDisplayInCenter;
    ...
    private void downloadPdf(final String pdfurl, final int page) {
        ...
        Log.e("download_info", "pdfurl: " + pdfurl + "保存在本地的地址  " + fileLocalUrl2 + "   show pdf 地址  " + showpdfurl + "  downloadurl " + downloadurl);
        filedownprogress.setVisibility(View.GONE);
        if (fileUtils.isFileExists(fileLocalUrl2)) {
            ...
            // 1:syncbook 2 document 3 syncroom 4 meeting
                                ...
                                newPath, new ServiceInterfaceListener() {
                                    ...
                                    public void getServiceReturnData(Object object) {
                                        ...
                                        if (ud != null) {
                                            ...
                                            if (1 == uploadao.getServiceProviderId()) {
                                                targetUrl = "https://s3." + uploadao.getRegionName() + ".amazonaws.com/" + uploadao.getBucketName() + "/" + newPath + "/" + filename;
                                                Log.e("check_url", "four,targetUrl:" + targetUrl + ", new path:" + newPath + ",currentAttachmentId:" + currentAttachmentId);
                                            } else if (2 == uploadao.getServiceProviderId()) {
                                                targetUrl = "https://" + uploadao.getBucketName() + "." + uploadao.getRegionName() + "." + "aliyuncs.com" + "/" + newPath + "/" + filename;
                                                Log.e("check_url", "five,targetUrl:" + targetUrl + ", new path:" + newPath + ",currentAttachmentId:" + currentAttachmentId);
                                            }
...
//        }
        ...
        Log.e("afterChangePageFunction", url);
...
//            getLineAction(currentAttachmentPage, !isPause);
    ...
    public void reflect(String result) {
        //自己的在html上面的operation,网页的回调
        Log.e("webview-reflect", result);
        String operation = Tools.getBase64(result).replaceAll("[\\s*\t\n\r]", "");
        Log.e("reflect", "operation:" + operation);
...
//        }
    ...
    public void autoChangeFileFunction(int diff) {
        Log.e("webview-autoChangeFile", diff + " " + documentList.size());
        if (documentList.size() <= 1) {
            return;
        }
        if (isHavePresenter()) {
            int currentPdfIndex = documentList.indexOf(currentShowPdf);
            if (diff == 1) {
                if (currentPdfIndex == documentList.size() - 1) {
                    currentPdfIndex = 0;
                } else {
                    currentPdfIndex++;
                }
                ...
                if (currentPdfIndex == 0) {
                    currentPdfIndex = documentList.size() - 1;
                } else {
                    currentPdfIndex--;
                }

            }
            currentShowPdf = documentList.get(currentPdfIndex);
            ...
            Log.e("check_url", "six,targetUrl:" + targetUrl + ", new path:" + newPath + ",currentAttachmentId:" + currentAttachmentId);
            if (diff == 1) {
        ...
        take_photo.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (!Environment.MEDIA_MOUNTED.equals(Environment
                    ...
                    Toast.makeText(SyncBookActivity.this, "请插入SD卡", Toast.LENGTH_SHORT).show();
                    return;
                }
                closeAlbum();
    ...
    private void moreActionPopupWindow() {
        ...
        moreactionPopup.getPopwindow(SyncBookActivity.this);
...
//        Toast.makeText(WatchCourseActivity3.this, mGroupId, Toast.LENGTH_LONG).show();
    ...
    private void openSaveVideoPopup() {
        ...
        favoritePopup.getPopwindow(SyncBookActivity.this);
    ...
    private void openDocumentPopup() {
        ...
        syncRoomDocumentPopup.setWebCamPopupListener(new SyncRoomDocumentPopup.WebCamPopupListener() {
            ...
            public void changeOptions(LineItem syncRoomBean,int position) { //切换文档
                ...
                LineItem currentShowPdf = syncRoomBean;
                currentShowPdf.setSelect(true);
                currentAttachmentId = currentShowPdf.getAttachmentID();  // itemId
                currentItemId = currentShowPdf.getItemId();
                targetUrl = currentShowPdf.getUrl();
                newPath = currentShowPdf.getNewPath();
                Log.e("check_url", "seven,targetUrl:" + targetUrl + ", new path:" + newPath + ",currentAttachmentId:" + currentAttachmentId);
                notifySwitchDocumentSocket(currentShowPdf, "1");
                wv_show.load("file:///android_asset/index.html", null);
            }
...
//                getWindow().getDecorView().setAlpha(0.5f);
    ...
    private void callMeOrLater(final int identity, String phoneNumber) {
        ...
        startLessonPopup.getPopwindow(SyncBookActivity.this, identity, phoneNumber);
    ...
    private void endDialog() {
        AlertDialog.Builder builder = new AlertDialog.Builder(SyncBookActivity.this);
        builder.setTitle("")
    ...
    private void openMySavePopup(final List<Favorite> favoriteList, final int type) {
        ...
        favoritePopup.getPopwindow(SyncBookActivity.this, favoriteList, type);
...
//                final String attachmentId = jsonObject.getString("vid");
    ...
    private void initpdfdata(final List<LineItem> documentList, String currentFileId) {
        ...
        Log.e("SyncRoomActivity", "initpdfdata，documentList：" + documentList.size() + ", currentFileId:" + currentFileId);
        if (documentList == null || documentList.size() == 0 || TextUtils.isEmpty(currentFileId)) {
            return;
        }
        currentShowPdf = null;
        if (currentFileId.equals("0")) {
        ...
        myRecyclerAdapter2.setMyItemClickListener(new MyRecyclerAdapter2.MyItemClickListener() {
            ...
            public void onItemClick(int position) {
                if (isHavePresenter()) {
                    ...
                    currentAttachmentId = currentShowPdf.getAttachmentID();
                    currentItemId = currentShowPdf.getItemId();
                    currentBlankPageNumber = currentShowPdf.getBlankPageNumber();
                    targetUrl = currentShowPdf.getUrl();
                    newPath = currentShowPdf.getNewPath();
                    Log.e("check_url", "eight,targetUrl:" + targetUrl + ", new path:" + newPath + ",currentAttachmentId:" + currentAttachmentId);
                    notifySwitchDocumentSocket(currentShowPdf, "1");
                    wv_show.load("file:///android_asset/index.html", null);
                }
            }
...
//        }
        ...
        Log.e("SyncRoomActivity", "currentShowPdf:" + currentShowPdf.toString());
        changedocumentlabel(currentShowPdf, documentList);
    ...
    public void uploadFile2(final LineItem attachmentBean) {
        ...
        lg.setprepareUploadingGetListener(new LoginGet.prepareUploadingGetListener() {
            ...
            public void getUD(Uploadao ud) {
                ...
                if (myRecyclerAdapter2 == null) {
                    myRecyclerAdapter2 = new MyRecyclerAdapter2(SyncBookActivity.this, documentList);
                    documentrecycleview.setAdapter(myRecyclerAdapter2);
                } else {
                    myRecyclerAdapter2.notifyDataSetChanged();
                }
    ...
    protected void initUIandEvent() {
        ...
        mSmallLeftAgoraAdapter = new RightAgoraAdapter(SyncBookActivity.this);
        ...
        mLeftAgoraAdapter = new DefaultAgoraAdapter(SyncBookActivity.this);
        ...
        mRightAgoraAdapter = new RightAgoraAdapter(SyncBookActivity.this);
        ...
        mBigAgoraAdapter = new BigAgoraAdapter(SyncBookActivity.this);
...
//        worker().getRtcEngine().enableAudioVolumeIndication(200, 3);
    ...
    protected void deInitUIandEvent() {
        Log.e("SocketService---------", "deInitUIandEvent");
        watchSyncroomInstance = false;
...
//        }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        Log.e("SocketService---------", "on new intent");
    }
...
//        Toast.makeText(this,"Synccroom onKeyDown:" + keyCode,Toast.LENGTH_SHORT).show();
...
//            Toast.makeText(this,"Synccroom KEYCODE_DPAD_LEFT",Toast.LENGTH_SHORT).show();
//            if (!goToPrePage()) {
//                goToPreDocument();
//            }
//            return false;
...
//            Toast.makeText(this,"Synccroom KEYCODE_DPAD_RIGHT",Toast.LENGTH_SHORT).show();
//            if (!goToNextPage()) {
//                goToNextDocument();
//            }
//            return false;
...
//            Toast.makeText(this,"Synccroom KEYCODE_MENU",Toast.LENGTH_SHORT).show();
//            if (syncroomll.getVisibility() == View.VISIBLE) {
//                syncroomll.setVisibility(View.GONE);
//                menu.setImageResource(R.drawable.icon_menu);
//            } else if (menu_linearlayout.getVisibility() == View.GONE) {
...
//            }
    ...
    private void goToPage(String pageNumber) {
        String changpage = "{\"type\":2,\"page\":" + pageNumber + "}";
        Message msg3 = Message.obtain();
        msg3.obj = changpage;
        msg3.what = 0x1109;
        handler.sendMessage(msg3);
    ...
    private boolean goToPrePage() {
        int page = Integer.parseInt(currentAttachmentPage);
        if (page > 0 && page <= pageCount) {
            currentAttachmentPage = (page - 1) + "";
            goToPage(currentAttachmentPage);
            return true;
        }
        return false;
    }
    ...
    private boolean goToPreDocument() {
        autoChangeFileFunction(-1);
        return false;
    }

    private boolean goToNextDocument() {
        autoChangeFileFunction(1);
        return false;
    }
...
//        Log.e("not_follow_respose", responsedata.toString() + "");
    ...
    private void handleFollowMovePage(String message) {
        ...
        if (msg_action.equals("SEND_MESSAGE")) {
            ...
            try {
                JSONObject jsonObject = new JSONObject(Tools.getFromBase64(d));
                if (jsonObject.getInt("actionType") == 8) {
                    Log.e("handleFollowMovePage", "msg:" + msg);
                }
            } catch (JSONException e) {
                e.printStackTrace();
            }
